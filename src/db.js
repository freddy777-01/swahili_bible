...
const dbcon = (exports.dbcon = new sqlite.Database(
  path.join(__dirname + "\\databases\\", "notes.db"),
  (err) => {
    if (err) {
      ...
      if (rows.length > 0) {
        ...
        rows.forEach((row) => {
          ...
          bkContents.innerHTML += `
              ...
              ${bookTitle(JSON.parse(row.bookmark).Kitabu)}, Sura: ${
            JSON.parse(row.bookmark).Sura
          }
              </span>
...
bkmark.onclick = () => {
  ...
  let sql = `INSERT INTO bookmarks(bookmark) VALUES('${JSON.stringify(
    bkObj
  )}')`;

  dbm.dbcon.all(
    "SELECT COUNT(bookmark) AS bkNum FROM bookmarks",
    (err, rows) => {
...
const allData = () => {
  ...
  document.querySelector(
    ".note .note-body .view-notes .view-notes-body"
  ).innerHTML = "";
...
const readNote = (id) => {
  ...
  dbm.dbcon.get(
    `SELECT title,notes FROM mynotes WHERE id='${id}'`,
    (err, row) => {
      if (err) {
        msgBox.innerHTML = `<p class="text-danger">${err}</p>`;
        setTimeout(() => {
